function [xsup,w,d,pos,timeps,alpha,obj]=svmclass_categorical(x,y,my_lambda, c,lambda,kernel,kerneloption,verbose, dataSta, dotProduct,span, alphainit, ps)
...
if verbose ~= 0 disp('building the kernel(or distance) matrix'); end;

ttt = cputime;

if nargin < 13
    ps  =  zeros(n,n);		% ps is the Kernel matrix
    ps = svmkernel_categorical(x, kernel, kerneloption, dataSta, dotProduct, my_lambda);   % If Kernel=='gaussian', the dotProduct matrix is computed in svmkernel_categorical.m %高斯核函数不需要参数dotProduct点积矩阵
end;
%----------------------------------------------------------------------
